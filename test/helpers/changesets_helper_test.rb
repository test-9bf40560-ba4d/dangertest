...
class ChangesetsHelperTest < ActionView::TestCase
  def test_changeset_user_link
    ...
    assert_equal %(<a dir="auto" href="/user/#{ERB::Util.u(changeset.user.display_name)}">#{changeset.user.display_name}</a>), changeset_user_link(changeset)
  ...
  def test_changeset_details
    ...
    user_link = %(<a dir="auto" href="/user/#{ERB::Util.u(changeset.user.display_name)}">#{changeset.user.display_name}</a>)
